function [x,y]=unsquareg(bs,s)
%UNSQUAREG	Gives geometry data for the unsquareg PDE model.
%
%   NE=UNSQUAREG gives the number of boundary segments
%
%   D=UNSQUAREG(BS) gives a matrix with one column for each boundary segment
%   specified in BS.
...
%
%   [X,Y]=UNSQUAREG(BS,S) gives coordinates of boundary points. BS specifies the
...
y(ii)=(0-(1))*(s(ii)-d(1,2))/(d(2,2)-d(1,2))+(1);
end
...
y(ii)=(0-(0))*(s(ii)-d(1,3))/(d(2,3)-d(1,3))+(0);
end
...
y(ii)=(1-(0))*(s(ii)-d(1,4))/(d(2,4)-d(1,4))+(0);
end
